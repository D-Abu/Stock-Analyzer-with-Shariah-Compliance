...
# Mini-watchlist for the live screener tab (to prevent timeouts)
    ...
    if analyze_button:
        with st.spinner(f"Compiling dashboard for {yf_ticker}..."):
            ...
            if bs is None or bs.empty or financials is None or financials.empty or hist_daily is None or hist_daily.empty:
                st.error(f"❌ Could not fetch complete data for {yf_ticker}.")
            else:
                ...
                # --- NEW: FUNDAMENTAL ANALYSIS BLOCK ---
                with st.container(border=True):
                    st.markdown("#### 🏢 Fundamental Analysis")
                    f_col1, f_col2, f_col3, f_col4 = st.columns(4)
                    
                    # Safely calculate ROE and ROCE
                    try:
                        ...
                        # 3 Years Ago (Index 3, or oldest available)
                    ...
                    st.info("⚠️ *Historical quarterly promoter holding is not provided by free YF APIs. Connect a premium API (like Screener.in) to populate this matrix.*")
                ...
                # --- AAOIFI BLOCK (Kept exactly the same) ---
                with st.container(border=True):
...
with tab2:
    ...
    st.write("Scan a predefined watchlist of top stocks based on Technical criteria. *(Note: Live scanning 5000+ stocks via free YF APIs will cause timeouts. Connect a database for full-market scans).*")
    
    # 3 Requested Filter Buttons
    ...
    if filter_1 or filter_2 or filter_3:
        ...
        for i, ticker in enumerate(WATCHLIST):
            try:
                # Update progress bar
                progress = int(((i + 1) / len(WATCHLIST)) * 100)
                my_bar.progress(progress, text=f"Scanning {ticker}...")
                
                stock = yf.Ticker(ticker)
                # Fetch daily and weekly data
                hist_d = stock.history(period="1y")
                hist_w = stock.history(period="2y", interval="1wk")
                ...
                if not hist_d.empty and not hist_w.empty:
                    ...
                    # Check Logic based on the button pressed
                    passed = False
